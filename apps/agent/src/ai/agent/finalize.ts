 ...
 *
 * Handles git commit, declaration extraction, cloud sync, and build.
 */
...
export async function finalizeSession(context: ChatContext): Promise<FinalizeResult> {
  const { project, branch, user } = context;
  ...
  // Create git commit using Git library
  ...
  if (commitHash) {
    await db.update(snapshots).set({ commitSha: commitHash }).where(eq(snapshots.id, snapshotId));
  }
