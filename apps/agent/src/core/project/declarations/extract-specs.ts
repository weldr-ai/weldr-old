...
async function extractDbModelSpecs(
  declarationName: string,
  filePath: string,
  sourceCode: string,
): Promise<z.infer<typeof dbModelDeclarationSpecsSchema> | null> {
  const result = await generateText({
    model: registry.languageModel("google:gemini-2.5-flash"),
    output: Output.object({ schema: dbModelDeclarationSpecsSchema }),
    prompt: `${promptMap["db-model"]}
...
Generate the complete db-model specification based on the actual implementation.
Focus on the declaration named "${declarationName}" if multiple declarations exist.`,
  });

  return result.output;
}

async function extractEndpointSpecs(
  declarationName: string,
  filePath: string,
  sourceCode: string,
): Promise<z.infer<typeof endpointDeclarationSpecsSchema> | null> {
  const result = await generateText({
    model: registry.languageModel("google:gemini-2.5-flash"),
    output: Output.object({ schema: endpointDeclarationSpecsSchema }),
    prompt: `${promptMap["endpoint"]}
...
Generate the complete endpoint specification based on the actual implementation.
Focus on the declaration named "${declarationName}" if multiple declarations exist.`,
  });
...
async function extractPageSpecs(
  declarationName: string,
  filePath: string,
  sourceCode: string,
): Promise<z.infer<typeof pageDeclarationSpecsSchema> | null> {
  const result = await generateText({
    model: registry.languageModel("google:gemini-2.5-flash"),
    output: Output.object({ schema: pageDeclarationSpecsSchema }),
    prompt: `${promptMap["page"]}
...
Generate the complete page specification based on the actual implementation.
Focus on the declaration named "${declarationName}" if multiple declarations exist.`,
  });
...
export async function extractSpecsFromCode(
  ...
  try {
    ...
    switch (specType) {
      case "db-model":
        specs = await extractDbModelSpecs(declaration.name, filePath, sourceCode);
        break;
      case "endpoint":
        specs = await extractEndpointSpecs(declaration.name, filePath, sourceCode);
        break;
      case "page":
        specs = await extractPageSpecs(declaration.name, filePath, sourceCode);
        break;
    }
